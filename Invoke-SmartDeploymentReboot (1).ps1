...
foreach ($Dep in $SelectedDeployments) {
    ...
    try {
        ...
        $Col = Get-CimInstance -ComputerName $SiteServer `
                   -Namespace "root\SMS\site_$SiteCode" `
                   -ClassName "SMS_Collection" `
                   -Filter "Name='$($Dep.CollectionName)'" `
                   -ErrorAction Stop

        if (-not $Col) {
            Write-Host "      Collection not found: $($Dep.CollectionName)" -ForegroundColor Yellow
            continue
        }

        # Collection members
        $Members = Get-CimInstance -ComputerName $SiteServer `
                       -Namespace "root\SMS\site_$SiteCode" `
                       -ClassName "SMS_CollectionMember_a" `
                       -Filter "CollectionID='$($Col.CollectionID)'" `
                       -ErrorAction Stop

        # Update compliance status (all machines — filter after)
        $StatusData = Get-CimInstance -ComputerName $SiteServer `
                          -Namespace "root\SMS\site_$SiteCode" `
                          -ClassName "SMS_UpdateComplianceStatus" `
                          -ErrorAction Stop

        # Build MachineName -> status lookup
        $StatusLookup = @{}
        ...
        foreach ($Member in $Members) {
            ...
            $IsFailed   = $Status -and $Status.LastErrorCode -and $Status.LastErrorCode -ne 0
            $IsUnknown  = -not $Status -or $Status.Status -eq 0
            $IsRequired = $Status -and $Status.Status -eq 2

            if ($IsFailed -or $IsUnknown -or $IsRequired) {
                ...
                # Add to maps — initialise lists if first time seeing this machine
                ...
                # Only add deployment if not already recorded (idempotent)
...
$DeduplicatedList = foreach ($KV in $MachineDeploymentMap.GetEnumerator()) {
    $Name    = $KV.Key
    $DepList = $KV.Value
...
} | Sort-Object FailedInCount -Descending, MachineName
