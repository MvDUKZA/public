...
foreach ($Dep in $SelectedDeployments) {
    ...
    try {
        # Get-CMDeploymentStatus returns aggregate (1 row per StatusType bucket).
        # For per-machine details we use Get-CMDeploymentStatusDetails which
        # returns one row per machine per status.
        #
        # StatusType values per the MECM SDK:
        #   1 = Success
        #   2 = In Progress
        #   3 = Requirements not met / Unknown
        #   4 = Error / Failed
        #   5 = Unknown (no status reported yet)

        # Pull per-machine details for Failed (4) and both Unknown buckets (3, 5)
        $FailedDetails  = Get-CMDeploymentStatusDetails -DeploymentId $Dep.DeploymentID -StatusType Error         -ErrorAction SilentlyContinue
        $UnknownDetails = Get-CMDeploymentStatusDetails -DeploymentId $Dep.DeploymentID -StatusType Unknown       -ErrorAction SilentlyContinue
        $ReqNotMet      = Get-CMDeploymentStatusDetails -DeploymentId $Dep.DeploymentID -StatusType Requirements -ErrorAction SilentlyContinue

        $FailCount    = 0
        $UnknownCount = 0

        # Process Failed machines
        foreach ($Entry in @($FailedDetails)) {
            if (-not $Entry) { continue }
            $Name = $Entry.DeviceName
            if (-not $Name) { $Name = $Entry.MachineName }
            if (-not $Name) { continue }

            $ErrorStr = if ($Entry.StatusDescription) {
        ...
        # Process Unknown + Requirements-not-met machines (treat as Unknown)
        foreach ($Entry in @($UnknownDetails) + @($ReqNotMet)) {
            if (-not $Entry) { continue }
            $Name = $Entry.DeviceName
            if (-not $Name) { $Name = $Entry.MachineName }
            if (-not $Name) { continue }
