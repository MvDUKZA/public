...
foreach ($Dep in $SelectedDeployments) {
    ...
    try {
        # Correct cmdlet chain per Microsoft docs:
        # 1. Get-CMSoftwareUpdateDeployment returns the assignment object
        # 2. Get-CMSoftwareUpdateDeploymentStatus returns summary rows
        #    (one per StatusType bucket — the numbers you see in the console tabs)
        # 3. Get-CMDeploymentStatusDetails drills into per-machine details
        #    for a specific bucket
        #
        # StatusType values from the MECM SDK (CORRECTED):
        #   1 = Success (Compliant)
        #   2 = In Progress
        #   4 = Unknown
        #   5 = Error (Failed)

        $Assignment = Get-CMSoftwareUpdateDeployment -DeploymentId $Dep.DeploymentID -ErrorAction Stop

        if (-not $Assignment) {
            Write-Host "      Assignment not found" -ForegroundColor Yellow
            continue
        }

        $Summary = Get-CMSoftwareUpdateDeploymentStatus -InputObject $Assignment -ErrorAction Stop
        ...
        # Diagnostic — show what status buckets came back
        $Breakdown = $Summary | Group-Object StatusType | Sort-Object Name |
                     ForEach-Object { "Type$($_.Name)=$($_.Count)" }
        Write-Host "      Summary buckets: $($Breakdown -join ' ')" -ForegroundColor DarkGray

        # For each bucket we care about (Error or Unknown), drill into per-machine details
        $FailCount    = 0
        $UnknownCount = 0

        foreach ($SummaryRow in $Summary) {

            # StatusType 5 = Error, 4 = Unknown
            if ($SummaryRow.StatusType -notin 4, 5) { continue }

            $IsFailed   = $SummaryRow.StatusType -eq 5
            $StatusLabel = if ($IsFailed) { "Failed" } else { "Unknown" }

            # Get per-machine details for this status bucket
            $Details = Get-CMDeploymentStatusDetails -InputObject $SummaryRow -ErrorAction SilentlyContinue
            ...
            foreach ($Entry in $Details) {
                ...
                # Build error string — prefer real status description if available
                $ErrorStr = if ($IsFailed) {
                                if ($Entry.StatusDescription) {
                                    "Failed: $($Entry.StatusDescription)"
                                } elseif ($Entry.ErrorCode) {
                                    "Failed 0x{0:X8}" -f $Entry.ErrorCode
                                } else {
                                    "Failed"
                                }
                            } else {
