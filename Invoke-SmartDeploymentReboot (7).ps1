...
foreach ($Dep in $SelectedDeployments) {
    ...
    try {
        # MECM versions differ on Get-CMDeploymentStatusDetails parameter names.
        # We use the WMI SMS_StatusMessage approach which is universally available.
        #
        # Per-machine deployment status lives in SMS_StatusMessage joined with
        # SMS_StatMsgInsStrings (the inserted strings that contain machine names).
        # But the cleanest universal approach is SMS_ClassicDeploymentAssetDetails
        # OR direct query of the v_ClassicDeploymentAssetDetails SQL view.
        #
        # Simplest reliable method: query SMS_StatMsgWithInsStrings WMI class
        # filtered by the deployment's AssignmentID.
        #
        # But even simpler — the MECM cmdlet Get-CMDeployment gives us the
        # AssignmentID and we can query SMS_CM_UpdateAssignmentStatus directly.
        #
        # Actually the MOST reliable approach is the one the MECM console uses:
        # query SMS_SUMDeploymentAssetDetails WMI class.

        $StatusData = Get-CimInstance -ComputerName $SiteServer `
                          -Namespace "root\SMS\site_$SiteCode" `
                          -ClassName "SMS_SUMDeploymentAssetDetails" `
                          -Filter "AssignmentID=$($Dep.DeploymentID)" `
                          -ErrorAction SilentlyContinue

        # Fallback to the Classic variant if SUM view not available
        if (-not $StatusData) {
            $StatusData = Get-CimInstance -ComputerName $SiteServer `
                              -Namespace "root\SMS\site_$SiteCode" `
        ...
        # Final fallback — WMI query on the general asset details view
        if (-not $StatusData) {
            $StatusData = Get-CimInstance -ComputerName $SiteServer `
                              -Namespace "root\SMS\site_$SiteCode" `
                              -Query "SELECT * FROM SMS_DeploymentAssetDetails WHERE AssignmentID=$($Dep.DeploymentID)" `
                              -ErrorAction SilentlyContinue
        }

        if (-not $StatusData) {
            Write-Host "      No status data retrievable for this deployment" -ForegroundColor Yellow
            continue
        }

        # Diagnostic — show what StatusType values we got
        $Breakdown = $StatusData | Group-Object StatusType | Sort-Object Name |
                     ForEach-Object { "Type$($_.Name)=$($_.Count)" }
        Write-Host "      Breakdown: $($Breakdown -join ' ')" -ForegroundColor DarkGray

        # Filter to Failed (4), Unknown (3), and no-status (5/0)
        # StatusType values:
        #   1 = Success
        #   2 = In Progress
        #   3 = Requirements not met / Unknown
        #   4 = Error / Failed
        #   5 = No Status / Unknown (never reported)
        $FailedMachines  = $StatusData | Where-Object { $_.StatusType -eq 4 }
        $UnknownMachines = $StatusData | Where-Object { $_.StatusType -in 3, 5, 0 -or $null -eq $_.StatusType }

        $FailCount    = 0
        $UnknownCount = 0

        foreach ($Entry in $FailedMachines) {
            $Name = $Entry.DeviceName
            if (-not $Name) { $Name = $Entry.MachineName }
            if (-not $Name) { $Name = $Entry.ResourceName }
            if (-not $Name) { continue }

            $ErrorStr = if ($Entry.StatusErrorCode -and $Entry.StatusErrorCode -ne 0) {
                            "Failed 0x{0:X8}" -f $Entry.StatusErrorCode
                        } elseif ($Entry.StatusDescription) {
        ...
        foreach ($Entry in $UnknownMachines) {
            $Name = $Entry.DeviceName
            if (-not $Name) { $Name = $Entry.MachineName }
            if (-not $Name) { $Name = $Entry.ResourceName }
            if (-not $Name) { continue }
            ...
            $MachineErrorMap[$Name].Add("$($Dep.SoftwareName) [Unknown]")
            $UnknownCount++
        }

        Write-Host "      Failed: $FailCount  |  Unknown: $UnknownCount" -ForegroundColor DarkGray
