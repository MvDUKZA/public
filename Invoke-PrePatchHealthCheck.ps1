...
if (-not (Test-Path $ModulePath)) {
    Write-Error "ConfigMgr PowerShell module not found at: $ModulePath"
    exit 1
}
...
Write-Host "  Querying collection members..." -ForegroundColor Cyan

try {
    $CollectionMembers = Get-CMCollectionMember -CollectionName $CollectionName -ErrorAction Stop
} catch {
    Write-Error "Failed to query collection '$CollectionName': $_"
    Pop-Location
    exit 1
}
...
if (-not $CollectionMembers -or $CollectionMembers.Count -eq 0) {
    Write-Warning "No members found in collection: $CollectionName"
    exit 0
}
