...
$Results = $MachineNames | ForEach-Object -Parallel {
    ...
    if (-not $Reachable) {
        $Result.HardIssues += "Machine offline/unreachable"
        return $Result
    }
    $Result.Online = $true
    ...
    try {
        $RebootData = Invoke-Command -ComputerName $MachineName -ErrorAction Stop -ScriptBlock {
    ...
    # ── 5. WMI Health (fallback if OS query failed earlier) ───────────────────
    if (-not $Result.WMIAccessible) {
        try {
            Get-CimInstance -ClassName Win32_OperatingSystem `
                -ComputerName $MachineName -ErrorAction Stop | Out-Null
            $Result.WMIAccessible = $true
        } catch {
            $Result.HardIssues += "WMI not accessible"
        }
    }
