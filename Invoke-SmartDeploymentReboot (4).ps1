...
foreach ($Dep in $SelectedDeployments) {
    ...
    try {
        # Query per-machine status for THIS specific deployment using the
        # deployment asset details view. This gives us StatusType per machine
        # which is the authoritative "did this machine succeed/fail/unknown"
        # for this specific deployment assignment.
        #
        # StatusType values:
        #   1 = Success (Compliant)
        #   2 = In Progress
        ...
        #   5 = Not Applicable
        #
        # We only want 3 (Unknown) and 4 (Error/Failed)

        $DeploymentStatus = Get-CimInstance -ComputerName $SiteServer `
                                -Namespace "root\SMS\site_$SiteCode" `
                                -ClassName "SMS_ClassicDeploymentAssetDetails" `
                                -Filter "AssignmentID=$($Dep.DeploymentID) AND (StatusType=3 OR StatusType=4)" `
                                -ErrorAction SilentlyContinue

        # If SMS_ClassicDeploymentAssetDetails isn't available in this MECM
        # version, fall back to SMS_SUMDeploymentAssetDetails
        ...
        if (-not $DeploymentStatus) {
            Write-Host "      Could not query per-deployment status — skipping this deployment" -ForegroundColor Yellow
            continue
        }

        $FailCount = 0
        foreach ($Entry in $DeploymentStatus) {
            ...
            $ErrorStr = switch ($StatusType) {
                3       { "Unknown" }
                4       {
                    if ($Entry.StatusErrorCode -and $Entry.StatusErrorCode -ne 0) {
