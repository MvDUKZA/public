...
# Uses Start-Job per machine with a hard 30-second timeout per machine.
# ForEach-Object -Parallel cannot forcibly kill hung WinRM/WMI threads —
# a job can be stopped cleanly regardless of what it's doing inside.

Write-Host "  Stage 2/2 — Full health check on $($ReachableMachines.Count) reachable machines..." -ForegroundColor Cyan
Write-Host "  Per-machine timeout: 30s  |  Throttle: $ThrottleLimit concurrent jobs" -ForegroundColor DarkGray
Write-Host ""

$TotalScan    = $ReachableMachines.Count
$Completed    = 0
$OnlineResults = [System.Collections.Generic.List[object]]::new()

# The scriptblock run inside each job — fully self-contained, no $using needed
$CheckScript = {
    param($MachineName, $DiskThresholdGB, $RebootAgeDaysThreshold)
    ...
    # ── CimSession — all WMI queries through one session ─────────────────────
    ...
    try {
        ...
        $RebootData = Invoke-Command -ComputerName $MachineName `
            ...
            $r = @{
                CBS = $false; WindowsUpdate = $false
                PostRebootReporting = $false; CCMClient = $false
                PFRTotalEntries = 0; PFRRealEntries = 0
            }
            ...
            if ($PFR) {
                $Src               = $PFR | Where-Object { $_ -match '\S' }
                $r.PFRTotalEntries = $Src.Count
                $r.PFRRealEntries  = @($Src | Where-Object {
                    try { Test-Path ($_ -replace '^\\\?\?\\','' -replace '^\\\?\\','') -EA SilentlyContinue } catch { $false }
                }).Count
            }
            try {
        ...
        if ($RebootData.PFRRealEntries -gt 0) { $HardSources.Add("PendingFileRename($($RebootData.PFRRealEntries)/$($RebootData.PFRTotalEntries))") }
    ...
    # Overall status
    if ($Result.HardIssues.Count -gt 0)      { $Result.OverallStatus = "BLOCKED" }
    elseif ($Result.SoftIssues.Count -gt 0)  { $Result.OverallStatus = "WARNING" }
    else                                      { $Result.OverallStatus = "READY"   }

    return $Result
}

# ── Job runner — throttled, with hard 30s per-machine timeout ────────────────
$ActiveJobs   = [System.Collections.Generic.List[object]]::new()
$MachineQueue = [System.Collections.Generic.Queue[string]]::new()
foreach ($m in $ReachableMachines) { $MachineQueue.Enqueue($m) }
$JobTimeout   = 30  # seconds — hard kill if machine doesn't respond in time

while ($MachineQueue.Count -gt 0 -or $ActiveJobs.Count -gt 0) {

    # Fill up to ThrottleLimit
    while ($ActiveJobs.Count -lt $ThrottleLimit -and $MachineQueue.Count -gt 0) {
        $Name = $MachineQueue.Dequeue()
        $Job  = Start-Job -ScriptBlock $CheckScript `
                    -ArgumentList $Name, $DiskThresholdGB, $RebootAgeDaysThreshold
        $ActiveJobs.Add([PSCustomObject]@{ Job = $Job; Machine = $Name; Started = (Get-Date) })
    }

    # Check for completed or timed-out jobs
    ...
    foreach ($Entry in $ActiveJobs) {
        ...
        if ($Entry.Job.State -in "Completed","Failed","Stopped") {
            ...
            if ($JobResult) {
                $OnlineResults.Add($JobResult)
            } else {
                # Job returned nothing (error) — add a stub result
                $OnlineResults.Add([PSCustomObject]@{
                    ...
                    WMIAccessible     = $false
                    TrustedInstaller  = "Unknown"
                    HardIssues        = @("Job failed/returned no data")
                    SoftIssues        = @()
            ...
            $Completed++
            if ($Completed % 25 -eq 0 -or $Completed -eq $TotalScan) {
                $Pct = [math]::Round(($Completed / $TotalScan) * 100)
                Write-Host "    ... $Completed of $TotalScan checked ($Pct%)" -ForegroundColor DarkGray
            }

        } elseif ($Elapsed -gt $JobTimeout) {
            # Hard timeout — stop and discard the hung job
            ...
            $OnlineResults.Add([PSCustomObject]@{
                ...
                WMIAccessible     = $false
                TrustedInstaller  = "Unknown"
                HardIssues        = @("TIMED OUT after ${JobTimeout}s — WinRM/WMI hung")
                SoftIssues        = @()
                OverallStatus     = "BLOCKED"
                CheckedAt         = (Get-Date -Format 'yyyy-MM-dd HH:mm:ss')
            })
            ...
            $Completed++
            Write-Host "    !! TIMEOUT: $($Entry.Machine) — killed after ${JobTimeout}s" -ForegroundColor Red
            if ($Completed % 25 -eq 0 -or $Completed -eq $TotalScan) {
                $Pct = [math]::Round(($Completed / $TotalScan) * 100)
                Write-Host "    ... $Completed of $TotalScan checked ($Pct%)" -ForegroundColor DarkGray
            }
    ...
    # Brief pause to avoid a hot CPU loop on the polling cycle
    if ($ActiveJobs.Count -ge $ThrottleLimit -or $MachineQueue.Count -eq 0) {
