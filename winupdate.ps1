...
.DESCRIPTION
    This script mounts the specified Windows image, adds all .msu and .cab update packages from a folder,
    commits the changes, and unmounts the image. It handles errors, logs progress, and ensures cleanup.

.PARAMETER WimFile
    Path to the install.wim file.

.PARAMETER UpdatesFolder
    Path to the folder containing .msu or .cab update files.

.PARAMETER MountDir
    Directory to mount the image (must be empty).

.PARAMETER ImageIndex
    Index of the image edition to patch (default: 1).
...
.EXAMPLE
    .\PatchWindowsImage.ps1 -WimFile "C:\ISO\sources\install.wim" -UpdatesFolder "C:\Updates" -MountDir "C:\Mount" -ImageIndex 1
...
try {
    ...
    $mountArgs = "/Mount-Wim /WimFile:`"$WimFile`" /Index:$ImageIndex /MountDir:`"$MountDir`""
