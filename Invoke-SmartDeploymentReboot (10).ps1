...
#region ── Step 3: Confirm machines ───────────────────────────────────────────

Write-Host "  Step 3/4 — Confirm machines to reboot..." -ForegroundColor Cyan
Write-Host ""

$SelectedMachines = $DeduplicatedList | Out-GridView `
    -Title "STEP 3 — CONFIRM MACHINES TO REBOOT  |  $($DeduplicatedList.Count) unique  |  Ctrl+Click to deselect  |  OK to reboot" `
    -OutputMode Multiple
...
Write-Host "  $($SelectedMachines.Count) machines confirmed for reboot processing." -ForegroundColor Green
Write-Host ""
...
$AddPrompt = Read-Host "  Add $($Results.Count) machines to '$MaintenanceCollectionName'? (Y/N)"
...
if ($AddPrompt -match '^[Yy]') {
    ...
    Write-Host "  Skipped — machines not added to any collection." -ForegroundColor DarkGray
    Write-Host ""
}
