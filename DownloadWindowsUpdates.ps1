...
.PARAMETER LogFile
    Path to the log file (default: $PSScriptRoot\logs\DownloadUpdates.log).

.PARAMETER StrictSearch
    Use strict (exact match) searching (default: $true).

.EXAMPLE
    .\DownloadWindowsUpdates.ps1 -SearchQueries @("KB5063878") -DownloadPath "C:\Updates" -Architecture "x64"
...
.NOTES
    ...
    Changelog: v1.4 - Updated default LogFile to use script's current folder ($PSScriptRoot\logs\DownloadUpdates.log) per new instructions.
               v1.3 - Updated .EXAMPLE to use "KB5063878" as the search query.
...
param (
    ...
    [string]$LogFile = "$PSScriptRoot\logs\DownloadUpdates.log",
