...
if ($DoReboot) {
    ...
    # ForEach-Object -Parallel cannot run from a CM drive. Ensure we're on
    # a real filesystem location before the parallel block runs.
    $CurrentLocation = (Get-Location).Path
    if ($CurrentLocation -notmatch '^[A-Za-z]:\\') {
        Write-Host "  Switching to C:\ (was on $CurrentLocation)" -ForegroundColor DarkGray
        Set-Location C:\
    }
...
if ($DoReboot) {
    ...
    $TotalToProcess = $SelectedMachines.Count
    Write-Host "  Processing $TotalToProcess machines (throttle: $ThrottleLimit)..." -ForegroundColor DarkGray
    Write-Host ""
    ...
    $ProgressCounter.TryAdd("Count", 0) | Out-Null
    ...
    $Results = $SelectedMachines | ForEach-Object -Parallel {
        ...
        # ── Step A: Ping check (fast fail for offline/blackholed machines) ───
        $PingOk = $false
        try {
            $PingOk = Test-Connection -ComputerName $MachineName -Count 1 -TimeoutSeconds 2 -Quiet -ErrorAction SilentlyContinue
        } catch {}

        if (-not $PingOk) {
            $Result.Outcome     = "Offline (ping failed)"
            $Result.RetryNeeded = $true
            $null = $ProgressCounter.AddOrUpdate("Count", 1, [System.Func[string,int,int]]{ param($k,$v) $v + 1 })
            return $Result
        }

        # ── Step B: TCP 135 reachability (DCOM/RPC endpoint) ────────────────
        try {
        ...
        if (-not $Result.Reachable) {
            $Result.Outcome     = "Pings but DCOM (port 135) unreachable"
            $Result.RetryNeeded = $true
            $null = $ProgressCounter.AddOrUpdate("Count", 1, [System.Func[string,int,int]]{ param($k,$v) $v + 1 })
            return $Result
        }

        # ── Step C: Open CimSession with DCOM protocol and hard timeout ─────
        # Get-CimInstance via -ComputerName uses WSMan by default which can
        # hang indefinitely. An explicit DCOM session with OperationTimeout
        # forces a hard cap per operation.
        $CimSess = $null
        try {
            ...
            $Result.Outcome     = "CimSession failed: $($_.Exception.Message)"
            $Result.RetryNeeded = $true
            $null = $ProgressCounter.AddOrUpdate("Count", 1, [System.Func[string,int,int]]{ param($k,$v) $v + 1 })
            return $Result
        }

        # ── Step D: Logged-on user check ────────────────────────────────────
        try {
            ...
            $Result.Outcome     = "WMI user query failed: $($_.Exception.Message)"
            $Result.RetryNeeded = $true
            Remove-CimSession -CimSession $CimSess -ErrorAction SilentlyContinue
            $null = $ProgressCounter.AddOrUpdate("Count", 1, [System.Func[string,int,int]]{ param($k,$v) $v + 1 })
            return $Result
        ...
        # ── Step E: No user — reboot (Flags 6 = forced reboot) ──────────────
        try {
            $Reboot = Invoke-CimMethod -CimSession $CimSess `
                          -ClassName Win32_OperatingSystem `
                          -MethodName Win32Shutdown `
                          -Arguments @{ Flags = [int32]6 } `
                          -ErrorAction Stop
        ...
        # Progress increment and occasional update
        $Done = $ProgressCounter.AddOrUpdate("Count", 1, [System.Func[string,int,int]]{ param($k,$v) $v + 1 })
        if ($Done % 5 -eq 0 -or $Done -eq $TotalToProcess) {
            Write-Host "    ... $Done of $TotalToProcess checked" -ForegroundColor DarkGray
        }
