...
Write-Host ("─" * 70) -ForegroundColor DarkGray
Write-Host "    1. Reboot only          (reboot unattended machines, no collection change)" -ForegroundColor White
Write-Host "    2. Add to collection    (add to '$MaintenanceCollectionName', no reboot)" -ForegroundColor White
Write-Host "    3. Both                 (add to collection AND reboot unattended machines)" -ForegroundColor White
Write-Host "    Q. Quit                 (exit without action)" -ForegroundColor DarkGray
Write-Host ""

$ActionChoice = $null
while ($ActionChoice -notin '1','2','3','Q','q') {
    $ActionChoice = Read-Host "  Enter choice (1/2/3/Q)"
}

if ($ActionChoice -match '^[Qq]') {
    Write-Host "  Exited without action." -ForegroundColor Yellow
    exit 0
}

$DoReboot     = $ActionChoice -in '1','3'
$DoCollection = $ActionChoice -in '2','3'
...
if ($DoCollection) {
    ...
    try {
        # Resolve the target collection by name
        $TargetCollection = Get-CimInstance -ComputerName $SiteServer `
                                -Namespace $Namespace `
                                -ClassName "SMS_Collection" `
                                -Filter "Name='$MaintenanceCollectionName' AND CollectionType=2" `
                                -ErrorAction Stop

        if (-not $TargetCollection) {
            Write-Host "  ERROR: Collection '$MaintenanceCollectionName' not found." -ForegroundColor Red
            Write-Host "  Make sure it exists and is a Device collection." -ForegroundColor Red
        } else {

            $CollectionID = $TargetCollection.CollectionID
            Write-Host "  Resolved CollectionID: $CollectionID" -ForegroundColor DarkGray
            Write-Host "  Adding machines..." -ForegroundColor Cyan

            # Get existing direct membership rules to skip machines already in collection
            $ExistingRules = Get-CimInstance -ComputerName $SiteServer `
                                 -Namespace $Namespace `
            ...
            $ExistingResourceIDs = @{}
            if ($ExistingRules) {
                foreach ($R in $ExistingRules) { $ExistingResourceIDs[$R.ResourceID] = $true }
            }
            ...
            # Get the writable collection object via WMI for the AddMembershipRule method
            $CollectionObj = Get-CimInstance -ComputerName $SiteServer `
                                 -Namespace $Namespace `
                                 -ClassName "SMS_Collection" `
                                 -Filter "CollectionID='$CollectionID'"
            ...
            foreach ($R in $Results) {
                ...
                # Resolve machine name -> ResourceID via SMS_R_System
                try {
                    $Resource = Get-CimInstance -ComputerName $SiteServer `
                                    -Namespace $Namespace `
                                    -ClassName "SMS_R_System" `
                                    -Filter "Name='$($R.MachineName)'" `
                                    -ErrorAction Stop |
                ...
                if (-not $Resource) {
                    Write-Host "    $($R.MachineName.PadRight(25)) NOT FOUND in MECM" -ForegroundColor DarkGray
                    $NotFound++
                    continue
                }
                ...
                if ($ExistingResourceIDs.ContainsKey($ResourceID)) {
                    Write-Host "    $($R.MachineName.PadRight(25)) already in collection" -ForegroundColor DarkGray
                    $AlreadyIn++
                    continue
                }

                # Build direct membership rule and add via AddMembershipRule method
                try {
                    $RuleArgs = @{
                        collectionRule = [CimInstance](
                            New-CimInstance -Namespace $Namespace `
                                -ComputerName $SiteServer `
                                -ClassName "SMS_CollectionRuleDirect" `
                                -Property @{
                                    ResourceClassName = "SMS_R_System"
                                    ResourceID        = [uint32]$ResourceID
                                    RuleName          = $R.MachineName
                                } `
                                -ClientOnly
                        )
                    }

                    $null = Invoke-CimMethod -InputObject $CollectionObj `
                                -MethodName "AddMembershipRule" `
                                -Arguments $RuleArgs `
                                -ErrorAction Stop

                    Write-Host "    $($R.MachineName.PadRight(25)) added" -ForegroundColor Green
                    $Added++
                    $ExistingResourceIDs[$ResourceID] = $true

                } catch {
                    Write-Host "    $($R.MachineName.PadRight(25)) FAILED: $($_.Exception.Message)" -ForegroundColor Red
                    $FailedToAdd++
                }
            }
            ...
            if ($Added -gt 0) {
                ...
                Write-Host "  To force evaluation now, right-click the collection in MECM console and choose 'Update Membership'." -ForegroundColor Yellow
                Write-Host ""
            }
        }
