...
$SyncHash  = [System.Collections.Concurrent.ConcurrentDictionary[string,int]]::new()
$SyncHash.TryAdd("Count", 0) | Out-Null
...
$OnlineResults = $ReachableMachines | ForEach-Object -Parallel {
    ...
    $Done = $SyncHash.AddOrUpdate("Count", 1, [System.Func[string,int,int]]{ param($k,$v) $v + 1 })
    if ($Done % 25 -eq 0 -or $Done -eq $TotalScan) {
        $Pct = [math]::Round(($Done / $TotalScan) * 100)
        Write-Host "    ... $Done of $TotalScan checked ($Pct%)" -ForegroundColor DarkGray
    }
