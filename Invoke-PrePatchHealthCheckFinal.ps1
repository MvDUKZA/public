...
$Results = $MachineNames | ForEach-Object -Parallel {
    ...
    # ── Result object ─────────────────────────────────────────────────────────
    # RebootSeverity: NONE / SOFT / HARD
    #   HARD = CBS, PendingFileRename (with real paths) — likely to block patching
    #   SOFT = WindowsUpdate, PostRebootReporting, CCMClient — common/stale on VDI,
    #          treated as warnings not blockers
    ...
    # ── 1. Connectivity ───────────────────────────────────────────────────────
    # Test TCP 5985 (WinRM HTTP) directly — faster and more accurate than ICMP.
    # Machines that block ping but allow WinRM will still be checked.
    # Hard 2-second timeout prevents the 4-minute WSMan retry loop.
    $Reachable = $false
    try {
    ...
    if (-not $Reachable) {
        $Result.HardIssues += "Machine offline/WinRM unreachable (port 5985 timeout)"
        return $Result
    }
    $Result.Online = $true

    # ── 2. Disk Space + Last Reboot Age (single WMI call) ────────────────────
    # Use a CimSession with explicit timeout so WMI calls don't hang
    ...
    # ── 3. Pending Reboot Detection (HARD vs SOFT) ────────────────────────────
    #
    #  HARD sources  (likely real, likely to block patching):
    #    CBS                  — component store has uncommitted changes
    #    PendingFileRename    — only counted if source paths still exist on disk
    #
    #  SOFT sources  (very common on VDI, often stale, warn only):
    #    WindowsUpdate        — WU stamped reboot required (stacks across cycles)
    #    PostRebootReporting  — WU telemetry flag, almost never a real blocker
    #    CCMClient            — MECM client reboot pending (common after any patch)

    $HardSources = [System.Collections.Generic.List[string]]::new()
    $SoftSources = [System.Collections.Generic.List[string]]::new()

    try {
        $SessionOpt = New-PSSessionOption -OpenTimeout 5000 -OperationTimeout 15000
        $RebootData = Invoke-Command -ComputerName $MachineName -SessionOption $SessionOpt -ErrorAction Stop -ScriptBlock {
            ...
            # HARD — CBS
            $result.CBS = Test-Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Component Based Servicing\RebootPending"

            # SOFT — Windows Update RebootRequired
            $result.WindowsUpdate = Test-Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\WindowsUpdate\Auto Update\RebootRequired"

            # SOFT — Post Reboot Reporting
            $result.PostRebootReporting = Test-Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\WindowsUpdate\Auto Update\PostRebootReporting"

            # HARD — PendingFileRenameOperations (validate paths are real)
            ...
            if ($PFR) {
                # Entries come in pairs: source path, destination path (empty = delete)
                # Filter to non-empty source paths only
                $SourcePaths = $PFR | Where-Object { $_ -match '\S' -and $_ -notmatch '^\s*$' }
                $result.PFRTotalEntries = $SourcePaths.Count

                # Check which source paths actually still exist on disk
                $RealPaths = $SourcePaths | Where-Object {
                    try {
                        $CleanPath = $_ -replace '^\\\?\?\\','' -replace '^\\\?\\',''
                        Test-Path $CleanPath -ErrorAction SilentlyContinue
                    } catch { $false }
                }
                $result.PFRRealEntries = @($RealPaths).Count
            }

            # SOFT — CCM Client reboot pending
            try {
                $CCMReboot = Invoke-CimMethod `
                                 ...
                                 -ErrorAction Stop
                $result.CCMClient = ($CCMReboot.RebootPending -or $CCMReboot.IsHardRebootPending)
            } catch {}

            return $result
        }

        # Populate HARD sources
        if ($RebootData.CBS)                              { $HardSources.Add("CBS") }
        if ($RebootData.PFRRealEntries -gt 0)            { $HardSources.Add("PendingFileRename($($RebootData.PFRRealEntries) real/$($RebootData.PFRTotalEntries) total)") }

        # Populate SOFT sources
    ...
    # ── 5. Overall Status ─────────────────────────────────────────────────────
    # BLOCKED  = any HARD issue (disk, CBS reboot, real PFR, disabled WU, CCM down, WMI broken)
    # WARNING  = SOFT issues only (stale reboot flags, reboot age, minor problems)
    # READY    = no issues
